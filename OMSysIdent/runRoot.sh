...
root -l -e 'gSystem->Load("../../install/lib/libOMSysIdent.so"); gROOT->ProcessLine(".include /usr/include/eigen3"); gROOT->ProcessLine(".include ../../install/include");' $1
# Below is a more complete list of libraries used in OMSysIdent, but this is currently not needed
#root -l -e 'gSystem->Load("../../3rdParty/FMIL/install/linux/lib/libfmilib_shared.so"); gSystem->Load("../../install/lib/libOMSimulatorLib.so"); gSystem->Load("/usr/local/lib/libceres.so"); gSystem->Load("../../install/lib/libOMSysIdent.so"); gROOT->ProcessLine(".include /usr/include/eigen3");' $1
